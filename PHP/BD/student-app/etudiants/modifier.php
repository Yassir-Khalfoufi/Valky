...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            // vérifier unicité e-mail (sauf pour cet étudiant)
            $check = $pdo->prepare('select id from etudiants where email = ? and id != ?');
            ...
            if ($check->fetch()) {
                ...
                $upd->execute([$nom, $prenom, $email, $date_naissance, $filiere, $id]);

                set_flash('success', 'Étudiant modifié avec succès.');
                header('Location: /etudiants/liste.php');
                exit;
            }
        } catch (PDOException $e) {
    ...
    // pré-remplissage depuis POST si erreurs
    ...
    <form method="post" action="">
        ...
        <div class="form-group">
            ...
            <input type="date" id="date_naissance" name="date_naissance"
                   value="<?= htmlspecialchars($etudiant['date_naissance']) ?>" required>
        ...
        <div class="form-group">
            ...
            <input type="text" id="filiere" name="filiere"
                   value="<?= htmlspecialchars($etudiant['filiere']) ?>" required>
