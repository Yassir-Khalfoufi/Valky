...
// Vérification session (retourne JSON si non connecté)
if (empty($_SESSION['user_id'])) {
    // Tenter le cookie remember me
...
try {
    switch ($action) {
        ...
        case 'add':
            ...
            $stmt = $pdo->prepare('
                INSERT INTO films (titre, realisateur, annee, genre, statut, affiche_url, user_id)
                VALUES (:titre, :real, :annee, :genre, :statut, :affiche, :uid)
            ');
            $stmt->execute([
                ':titre'   => $titre,
        ...
        case 'list':
            ...
            $sql = 'SELECT * FROM films WHERE ' . implode(' AND ', $where) . " ORDER BY $orderBy DESC";
