...
function filmCardHTML(f) {
  const poster = f.has_poster
    ? `<img src="poster.php?id=${f.id}&t=${f.updated_at||''}" alt="${escHtml(f.titre)}" loading="lazy">`
    ...
    <div class="film-card" data-id="${f.id}">
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          ...
          <button class="btn btn-sm" style="background:var(--bg3);color:var(--accent)" data-action="edit" data-id="${f.id}" title="Éditer">✏️</button>
...
grid.addEventListener('click', async (e) => {
  ...
  const realisateur = meta ? meta.children[0]?.textContent : '';
  const annee = meta ? meta.children[1]?.textContent : '';
  const genre = meta ? meta.children[2]?.textContent : '';
  ...
  modal.innerHTML = `
    <div class="edit-form">
      <h3>✏️ Éditer le film</h3>
      <form id="edit-film-form">
        <div class="form-group">
          <label>Titre *</label>
          <input type="text" class="edit-input" id="edit-titre" value="${escHtml(titleText)}" required>
        </div>
        <div class="form-group">
          <label>Réalisateur</label>
          <input type="text" class="edit-input" id="edit-real" value="${escHtml(realisateur)}">
        </div>
        <div class="form-group">
          <label>Année</label>
          <input type="number" class="edit-input" id="edit-annee" value="${annee || ''}">
        </div>
        <div class="form-group">
          <label>Genre</label>
          <input type="text" class="edit-input" id="edit-genre" value="${escHtml(genre)}">
        </div>
        <div class="form-group">
          <label>Affiche (JPG/PNG/WebP, max 3 Mo)</label>
          <input type="file" id="edit-affiche" accept="image/jpeg,image/png,image/webp,image/gif">
        </div>
        <div class="form-actions">
          <button type="submit" class="btn btn-primary" style="flex:1">Sauvegarder</button>
          <button type="button" class="btn" style="flex:1;background:var(--bg3);color:var(--muted)" id="cancel-edit-btn">Annuler</button>
        </div>
      </form>
    </div>
  `;
  ...
  document.getElementById('edit-titre').focus();
  
  // Submit form
  document.getElementById('edit-film-form').addEventListener('submit', async (ev) => {
    ev.preventDefault();
    const fd = new FormData();
    fd.append('action', 'edit');
    fd.append('id', id);
    fd.append('titre', document.getElementById('edit-titre').value);
    fd.append('realisateur', document.getElementById('edit-real').value);
    fd.append('annee', document.getElementById('edit-annee').value);
    fd.append('genre', document.getElementById('edit-genre').value);
    
    const file = document.getElementById('edit-affiche').files[0];
    if (file) fd.append('affiche', file);
